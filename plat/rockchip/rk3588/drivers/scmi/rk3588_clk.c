...
static unsigned long clk_scmi_gpu_get_rate(rk_scmi_clock_t *clock)
{
	...
	if ((mmio_read_32(CRU_BASE + CRU_CLKSEL_CON(158)) & 0x4000) != 0) {
		// gpu clock (bit1) and mux clock (bit4) must be gated on (0) via clk gate 66
		if ((mmio_read_32(CRU_BASE + CRU_CLKGATE_CON(66)) & 0x12))
			return 0;
		// BIU_GPU (bit0) must be 'not idle' (0) via PMU_BIU_IDLE_STS0
		if ((mmio_read_32(PMU0_BASE + 0x8120) & 0x1))
			return 0;
		// pd_gpu_bisr_pwr_repair_stat (bit1) must be 'Complete' (1) via PMU_BISR_STS4
		if (!(mmio_read_32(PMU0_BASE + 0x8290) & 0x2))
			return 0;
		udelay(2);
